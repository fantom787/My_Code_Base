...
void solve(int testcase)
{
    ...
    auto update = [&](int node, int l, int r, int ql, int qr, int val, auto &&update) -> void
    {
        if (r < ql || l > qr)
        {
            return;
        ...
        int mid = (l + r) / 2;
        update(2 * node, l, mid, ql, qr, val, update);
        update(2 * node + 1, mid + 1, r, ql, qr, val, update);
    };
