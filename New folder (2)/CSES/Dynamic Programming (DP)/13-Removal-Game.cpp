...
void solve(int testcase)
{
    ...
    auto f = [&](int i, int j, bool bari, auto &&f) -> int
    {
        if (i > j)
        {
            return 0;
        ...
        int op1 = (bari ? 0 : a[i]) + f(i + 1, j, bari ^ 1, f);
        int op2 = (bari ? 0 : a[j]) + f(i, j - 1, bari ^ 1, f);
        int ans = (bari ? min(op1, op2) : max(op1, op2));
        return dp[i][j] = ans;
    };
    cout << f(0, n - 1, 0, f) << endl;
}
