...
void solve(int testcase)
{
    ...
    {
        int op1 = a[i].ad;
        int j = lower_bound(all(rr), a[i].l) - rr.begin();
        j--;
        op1 += (j >= 0 ? dp[j] : 0);
        int op2 = (i - 1 >= 0 ? dp[i - 1] : -INF);
        dp[i] = max(op1, op2);
    }
    cout << dp[n - 1] << endl;
    //---------------dp[i] advantage that we can get at ith project
    /*
    int n;
    cin >> n;
    vector<s> a(n);
    ...
    for (int i = n - 1; i >= 0; i--)
    {
        int op1 = (i + 1 < n ? dp[i + 1] : -INF);
        int op2 = a[i].ad;
        int j = upper_bound(all(ll), a[i].r) - ll.begin();
        op2 += (j < n ? dp[j] : 0);
        dp[i] = max(op1, op2);
    }
    cout << dp[0] << endl;
