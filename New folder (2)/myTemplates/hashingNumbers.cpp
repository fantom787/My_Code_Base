...
public:
    ...
    hstring(vector<int> x)
    {
        while (pow[0][0].size() < x.size())
            for (int i = 0; i < 2; i++)
                for (int j = 0; j < 2; ++j)
                    pow[i][j].push_back(pow[i][j].end()[-1] * (j == 0 ? base : inv[i]) % mod[i]);
        pre[0] = pre[1] = {x[0]};
        for (int i = 1; i < (int)x.size(); i++)
            for (int j = 0; j < 2; ++j)
                pre[j].push_back((x[i] * pow[j][0][i] + pre[j].end()[-1]) % mod[j]);
    }
    hvalue interval(int l, int r)
    {
        if (l == 0)
            return {r + 1, (pre[0][r] << 32ll) + pre[1][r]};
        return {r - l + 1, (((pre[0][r] - pre[0][l - 1] + mod[0]) * pow[0][1][l] % mod[0]) << 32ll) + (pre[1][r] - pre[1][l - 1] + mod[1]) * pow[1][1][l] % mod[1]};
    }
};
